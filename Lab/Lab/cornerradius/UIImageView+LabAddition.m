...
@property (nonatomic, assign) BOOL mgtCornerRadius;

@end

@implementation UIImage (cornerRadius)

- (BOOL)mgtCornerRadius {
    return [objc_getAssociatedObject(self, @selector(mgtCornerRadius)) boolValue];
}

- (void)setMgtCornerRadius:(BOOL)mgtCornerRadius {
    objc_setAssociatedObject(self, @selector(mgtCornerRadius), @(mgtCornerRadius), OBJC_ASSOCIATION_COPY_NONATOMIC);
}

@end

/////////////////////////////////////////////////////////////////////
@interface MGTImageObserver : NSObject
...
@implementation MGTImageObserver
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString*, id> *)change context:(void *)context {
    if ([keyPath isEqualToString:@"image"]) {
        ...
        if (![newImage isKindOfClass:[UIImage class]] || newImage.mgtCornerRadius) {
            return;
        }
...
/////////////////////////////////////////////////////////////////////
@implementation UIImageView (LabAddition)

- (CGFloat)mgtCornerRadius {
    return [self imageObserver].cornerRadius;
}

- (void)setMgtCornerRadius:(CGFloat)mgtCornerRadius {
    [self imageObserver].cornerRadius = mgtCornerRadius;
}

- (MGTImageObserver *)imageObserver {
    MGTImageObserver *imageObserver = objc_getAssociatedObject(self, @selector(imageObserver));
    if (!imageObserver) {
        imageObserver = [[MGTImageObserver alloc] initWithImageView:self];
